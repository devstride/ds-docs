...
Job roles are available through the API and MCP alongside person assignments. For example:

- *"Show work assigned to Engineer that is still waiting for a person."*
- *"Assign this item to Jane and preserve Engineer if she holds that role."*
- *"Show the roles each member can fill, in their fallback order."*
- *"Summarize recorded time by job role, keeping older unclassified time separate."*

The same permissions and person/role compatibility rules apply as in the app. A job role never grants item access. Catalog updates use the last-read catalog revision to prevent one administrator overwriting another's changes; saved roles are retired rather than deleted. See [Managing job roles](/workspace-administration/settings/organization/org-profile#managing-job-roles).
