...
Ticket Time Sweep only appears once Service Desk is enabled, and it only ever looks at **tickets** — items with a requester attached. Ordinary work items are never swept. Note that every email to a support address now opens a ticket, including one from your own team: those are swept like any other, so the seeded auto-close rule will close a colleague's ticket and email them the closure notice. See [Enabling Service Desk](/major-modules/service-desk/enabling-service-desk).
::
...
| **Hours Since Customer Reply** | The customer's last reply. A reply a colleague forwards in on the customer's behalf counts as the customer's, so this clock resets from their words rather than the forward. |
