...
- **A new request that passes the inbound checks** creates a new ticket — from anyone, including a member of your own organization. The email's subject becomes the ticket title, the body becomes the ticket's **Customer** description, the sender becomes (or is matched to) a [requester](/major-modules/service-desk/requesters-and-companies), and your **Notify on new ticket** defaults are applied. A message a colleague forwards in is the exception — see [below](#when-someone-on-your-team-emails-or-forwards-a-message-in).
...
- **Recognized delivery-failure notices** are held in the [Suspended queue](/major-modules/service-desk/suspended-messages) as auto-responses, so a bounced email does not open another ticket.
...
::alert{type="note" title="Encrypted and unencrypted mail"}
DevStride accepts inbound email whether or not the sending mail server encrypts the connection. Encryption is used whenever the sender offers it, and every message — encrypted or not — is scanned for spam and viruses and passes the same sender checks before it can become a ticket. Before September 2026, mail from servers that could not encrypt was refused.
::
...
- The message **positively passed DMARC** — a stricter bar than the [Suspended queue](/major-modules/service-desk/suspended-messages) applies, which only holds mail that explicitly *fails* a check.
...
::alert{type="note" title="If forwards are never credited, check your own DMARC first"}
Crediting needs an explicit DMARC **pass**. A sending domain that publishes no DMARC record at all produces neither a pass nor a fail — so mail from it moves through the Suspended queue normally, but its forwards are never credited, and nothing on screen explains the difference. If your team's forwards are consistently landing under the forwarder, confirm your own sending domain publishes a DMARC record before looking anywhere else.
::
