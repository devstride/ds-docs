...
::alert{type="note" title="When an email was too long to store"}
DevStride stores up to 100,000 characters of an email. If a customer's email was longer, the ticket (or the reply it became) is stored with the end cut off and says so, both to your team on the ticket and to the customer in the portal: *"This email was longer than we can store, so the end of it is missing."* Ask the customer for the rest if you need it. A reply is only marked when the customer's own new text was cut off, not when only the quoted earlier messages below it were.
::
