...
Open **Service Desk → Inbox** for your triage queue. Every email that reaches a channel address opens a ticket here, whoever sent it — see [Tickets from your own team, and forwarded mail](#tickets-from-your-own-team-and-forwarded-mail). It's the familiar items table, automatically scoped to tickets — a locked **Tickets only** filter keeps non-support work out, so you're always looking at customer requests and nothing else.
...
- **The customer.** Pick an existing [requester](/major-modules/service-desk/requesters-and-companies), or create one during the conversion if they are not on file yet.
- **What the customer should see.** The item's existing description is your team's, and it may not be written for a customer. You choose the customer-visible description as part of converting, so nothing internal is exposed by accident.

From the moment it converts, the item behaves like any other ticket: SLA clocks start, the customer's replies thread onto it, and it appears in their [Customer Support Portal](/major-modules/service-desk/customer-support-portal) alongside requests they raised themselves.

You can convert from three places:

- the **right-click menu** on an item, under **Edit**
- the item's **side panel**
- **bulk edit**, to convert several items at once — from the items list, boards, map, or portfolio

::alert{type="info" title="Who can convert"}
Converting requires the **Manage Requesters** permission and Service Desk enabled for your organization. Both are checked on every surface above, so the option only appears where you can actually use it.
::
...
When DevStride can't safely tell who the customer is, it credits the ticket to the colleague who forwarded it and keeps the forwarded text as an internal note instead — so glance at the Requester pane before you reply. [Email Channels & Setup](/major-modules/service-desk/email-channels#when-someone-on-your-team-emails-or-forwards-a-message-in) sets out exactly when each happens.
...
::alert{type="tip" title="Treat automatic priority as a triage head start"}
The result is meant to help urgent requests stand out quickly. Your team can still review and change the priority exactly as it would on any other work item.
::
...
This merges two **tickets** (the same request reported twice). To combine two **requester records** for the same person writing from different addresses, use the Merge action on the requester instead — see [Requesters & Companies](/major-modules/service-desk/requesters-and-companies). A requester whose address belongs to a member of your organization can't be merged or erased at all; the reason is on that page.
::
...
- **Customer** — what the requester wrote, and what they can see. The body of their original email lands here — unless it arrived as a forward DevStride couldn't safely attribute, in which case the message is kept as an internal note instead (see [Tickets from your own team, and forwarded mail](#tickets-from-your-own-team-and-forwarded-mail)).
...
### Showing your internal description to the customer

Sometimes the split is more work than it is worth — you have written a clear internal description and you would rather the customer simply read it. Each ticket has a **Show internal description to the customer** switch that mirrors the internal description into the customer-visible one every time the ticket is saved.

It is **off by default**, and deliberately so: the internal description is private until you decide otherwise. Turning it on requires **Send Public Replies** or **Manage Requesters** — but anyone who can edit the item may turn it **off**, so a mirror can always be stopped by whoever notices it should not be running.

The mirrored text is sanitized before the customer sees it, and the mirror applies on every save while it is on. If you later edit the internal description, the customer-visible one follows.
...
1. An email reaches your support address → a **ticket** appears in the Inbox with the message as the Customer description. A message a colleague forwarded in is credited to the original customer (see [Tickets from your own team, and forwarded mail](#tickets-from-your-own-team-and-forwarded-mail)).
