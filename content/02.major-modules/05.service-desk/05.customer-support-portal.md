...
**Contacts of more than one company.** One person can be a contact for several of your customers. They sign in to each company's portal separately and see only that company's requests there. At a portal they don't belong to yet, the portal's own [access mode](#who-can-sign-in-access-modes) decides whether they're let in, exactly as for anyone else. Being let in adds that company to their record but never changes their main company. Your team can also add or remove a person's companies directly: see [Requesters & Companies](/major-modules/service-desk/requesters-and-companies#a-requester-in-more-than-one-company).

**Opened the wrong company's portal?** If a requester belongs to another company and this portal's access mode doesn't let them in, DevStride can email them directions to their own company's portal. This guidance email contains no sign-in code or one-time login link; they still sign in at the correct portal. It does not move their account or give them access to another company. The on-screen confirmation stays the same to protect customer privacy.
...
| **Invite only** | Only requesters who already belong to the company, as their main company or one your team added. |
...
Signing in lands a customer on **My Requests** — a list of every request they've raised with you for this company, with a **search box** and a **status filter** so a customer with a long history can find the one they mean. Each row shows the request's title and where it stands in plain customer-facing terms, so "is anyone looking at this?" is answered without opening a single ticket.

### Following a request

Opening a request shows its **conversation** — the same public thread your team maintains from the ticket, with your agents' public replies and the customer's own messages in one place. A status chip at the top (for example **Awaiting your reply**) tells the customer at a glance whether the next move is theirs or yours, and it stays in step with the thread as replies land. If a customer replies to one of your team directly and that colleague forwards the reply in, it appears here as the customer's own message too, so the thread they see stays complete. If an email the customer sent was too long to store in full, the request or reply shows *"This email was longer than we can store, so the end of it is missing."* so they know to send the rest.
...
- **Attach files** — screenshots, documents, logs and screen recordings. Videos (MP4, MOV or WebM) can be up to **100 MB**; every other file type can be up to **25 MB**. Every uploaded file is **automatically scanned for malware** before your team ever sees it; a file that fails the scan is discarded and never reaches the ticket.
...
The portal's **Submit request** page lists the request forms you've chosen to publish there, so a signed-in customer can raise a new request without leaving the portal — no email required, and the submission files itself exactly as [a form submission always does](/major-modules/service-desk/request-forms). You control which forms appear: listing a form in the portal is a per-form setting, and any enabled form can be listed — public or private, file-upload fields included. See [Request Forms](/major-modules/service-desk/request-forms#forms-in-the-customer-support-portal) for how to publish one.
