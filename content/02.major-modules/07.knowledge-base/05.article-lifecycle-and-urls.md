...
Every one of these moves is recorded in the article's **History** panel, with who made it and when — so a published-then-unpublished-then-republished article carries that trail rather than just its current state. See [Article history and restore](/major-modules/knowledge-base/article-history-and-restore).
...
Because autosave fires as you work, consecutive edits by the same person within 15 minutes are grouped into a **single** History entry rather than one per save.
