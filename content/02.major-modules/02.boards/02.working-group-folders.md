...
When you open an individual board, each status column shows its own subtotals, and the board displays a **Board Totals** footer summarizing items, members, points, and time. See the board view documentation for details.
::

### Permanently deleting an archived board or folder

Archiving hides a board or folder; **Permanently delete** removes it for good. The option appears only on an **archived** perpetual board (its card's kebab menu, with **Show Archived** on) or an **archived** folder (its context menu), and only when you can delete everything inside it.

- **Your work items are kept.** Items, subtasks and workstreams that were on the deleted boards stay in their workstreams; only their board placement is cleared, and an item's status may adjust to its team's or organization's workflow.
- **Cycle boards go with their folder.** To remove individual cycles, permanently delete the archived folder that holds them.
- **Deletion runs in the background.** Once it starts you can close the dialog and follow progress in **Jobs**. If it stops, **Retry deletion** continues from the last saved step.
- **It cannot be undone.**
...
A folder's context menu (right-click a row, or use its kebab) offers permission-gated actions including **New Folder**, **Move**, **Edit**, **Archive** / **Unarchive**, **Permanently delete** (archived folders only), and **Permissions**. The options you see depend on your access to that folder.
::
