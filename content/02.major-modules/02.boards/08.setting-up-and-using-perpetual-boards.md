...
description: "How to create and use perpetual (evergreen) boards in DevStride, including who can create them."
---
...
::alert{type="note" title="Who can create perpetual boards"}
Users with the **Member** role can create perpetual boards directly from the Boards page — board creation is no longer limited to Admins. Boards inherit permissions from their parent folder, so anyone with Editor (update) access to a folder can add a perpetual board inside it. **Cycle boards** are different: they are generated from a cadence and configured through cadence settings, which remain Admin-managed.
::
...
* If you need a [new folder](/boards/working-group-folders), create it first. Note that creating a top-level folder requires the **Admin** role — once a folder exists and you have Editor (update) access to it, you can add boards inside it as a Member.
...
::alert{type="info" title="Perpetual vs. Cycle Boards"}
The **New** button in the **Perpetual Boards** section opens the board creation dialog. The **New** button in the **Cycle Boards** section instead takes you to cadence settings, where time-based boards are configured. Use the Perpetual Boards section for evergreen boards.
::
...
* **Name** (required) — Enter a name for the board. The name must be at least 3 characters.
...
::alert{type="caution" title="Keep it evergreen"}
To keep this a perpetual board, do not assign a cadence (sprints, cycles, PIs). That's what makes it evergreen — it runs continuously with no time box.
::
...
Archive completed work periodically to keep the board clean — or let an automation do it. The **Item Stays Done on Board** trigger fires once an item has sat in Done on a board you choose for a set time (**Time spent Done on this board**), and can then move the item to another board or take it off this one. It applies to items that reach Done after the rule is on; reopening an item or moving it to another board starts the wait again, and a rule acts once per stretch in Done.

For a step-by-step rule, see [remove completed work after a delay](/workspace-administration/automations/setting-up-basic-automations#remove-completed-work-from-a-board-after-a-delay).
