...
See the [Setting Up Perpetual Boards](/boards/setting-up-and-using-perpetual-boards) and [Using Cycle Boards](/major-modules/boards/using-cycle-boards) articles for more on how to navigate to the board you want to see.
...
##### Quickly reorder a card to the top or bottom of a status

Dragging is the usual way to reorder cards within a status column, but you can also jump a card straight to the top or bottom of its status.

Right-click any card you can edit and choose one of the two options in the menu that appears:

* **Move to Top** — sends the card to the top of its status.
* **Move to Bottom** — sends the card to the bottom of its status.

This is a faster alternative to dragging when a status column is long. Each option is disabled when it would have no effect — **Move to Top** is disabled when the card is already first in its status, and **Move to Bottom** is disabled when the card is already last.
...
The top-level rows you see are scoped to the current board, but expanding a row can pull in child items that live on **other** boards (unless **Show Matching Children Only** is turned on, which keeps subtree results within the same filters). The Table view is a flat data grid, so turning on hierarchy does not change the board's statuses or columns — those belong to the Card and 2D views.
::
