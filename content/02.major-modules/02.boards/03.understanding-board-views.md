...
### Organize rows or columns by Persona

Choose **Persona** in either the **Rows** or **Columns** control above the 2D board. For example, use Persona rows and Status columns to see the work planned for Engineers, QA Analysts, and other specialties. Grouping uses the persona chosen on each item, rather than all the personas its assignee can perform.

Dragging an item into a different persona group changes its chosen persona. A compatible assignee stays assigned; an assignee who cannot perform that persona is cleared, leaving work planned for the persona and waiting for a person. The same assignment permissions still apply.
