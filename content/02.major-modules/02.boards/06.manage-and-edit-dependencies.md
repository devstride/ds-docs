...
- A **View Graph** button opens the [Dependency Graph](/workstreams-and-work-items/dependency-graph), where you can explore relationships visually across items.
- Three sections list the relationships by type: **Dependencies**, **Related**, and **Dependents**.

Archived items stay in the list with an **Archived** label, so an item related only to archived work still shows its relationships. If a related item has been deleted, or is somewhere you don't have access to, its row shows the item number and **Not available — it was deleted, or you don't have access to it**; you can remove the relationship from that row. These rows count toward the Summary's totals, but never as completed.
