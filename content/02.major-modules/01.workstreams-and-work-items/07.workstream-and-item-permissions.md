...
## What you see in filtered lists

You can filter a list of workstreams by something about the **items inside them** — their assignee, team, tag, or a custom field value. A workstream appears in that filtered list only when an item **you can read** matches the filter.

A workstream you can read is never hidden from you because of this. What changed is the reverse case: a workstream no longer surfaces on the strength of a child item you have been denied. Previously it could, which told you such an item existed and carried that value without letting you open it.

The practical effect is that two people can run the same filter and see different numbers of workstreams, and the person with narrower access sees fewer. That is the filter reporting only what that person can actually reach. A role holding the **Manage all items & workstreams** override below sees every match, as it does everywhere else.
