...
<!-- TWIN COPY — keep in sync. An identical copy of this page lives under Admin Onboarding ▸ Populating and Managing Workstreams (01.getting-started/01.admin-onboarding-getting-started/10.populating-and-managing-workstreams/01.import-templates.md). Update BOTH together. -->
...
In the **Map Value** module, open the **ellipsis (⋯) menu** at the top of a sub-workstream column and choose **Export as Template (Excel)**.
...
<!-- TODO: screenshot — the workstream column three-dot menu showing Export as Template (Excel) and Import Template (Excel) -->
...
2. Open the **ellipsis (⋯) menu** at the top of that column and choose **Import Template (Excel)**.
...
::alert{type="note" title="Where Import Template (Excel) appears"}
**Import Template (Excel)** is available only from the ellipsis menu on a **nested workstream** column in Map Value — never on the root **Core Workstreams** column — and only when you have edit access to that workstream.
::
...
- **Teams** by name
- **Member roles** by role name, on the template's **Users** sheet
...
::alert{type="info" title="How member roles are assigned"}
The **Role** column on the **Users** sheet is matched against the roles in the organization you're importing *into*. If the role doesn't exist there — or the cell is blank — that person is given your organization's **default invite role** rather than the import failing.

Two things worth knowing: a template can never grant the **Owner** role, and importing never changes the role of someone who is *already* a member. Roles are only applied when a new member is created.
::
