...
description: "Add items to your workstreams and nest them by item type to build out your work hierarchy."
---
...
To turn existing notes or documents into a batch of items at once, see [Creating Items with AI](/workstreams-and-work-items/ai-item-creation). To build out a whole workstream from a reusable spreadsheet, see [Import Templates](/workstreams-and-work-items/import-templates). To navigate, bulk-edit, and quick-add work across your whole hierarchy, see [Using the Map](/workstreams-and-work-items/using-the-map).
::
...
The advantage of a Plan Value workstream map is that you can see all the work that goes into Product A, for example. All of the things—or work—in Product A rolls up into it. You will be able to see estimates for all the work, the level of effort required, completion progress, defects, etc. for Product A. New items inherit access from the location they're created in — see [Workstream & Item Permissions](/workstreams-and-work-items/workstream-and-item-permissions).
...
At the most granular level, work can be assigned to a team member or to a persona waiting for a person.
::

## Assigning a person or persona

Assignment is optional when you create an item. Open its **Assignee** control later to choose from **People** or **Personas**. The same assignment choices are available for subtasks, item-table and Gantt columns, and bulk edits.

Choose a persona such as Engineer when you know what kind of work is needed but have not chosen the person. A persona-only item counts as assigned, while still showing that it is waiting for a person.

The person and persona stay compatible. Assigning Jane preserves a selected persona she holds; otherwise her first active persona is used. Choosing a persona Jane does not hold clears her from the assignment. **Clear**, beside the chooser title, clears both the person and persona.

Administrators define personas and each member's eligible personas in [organization settings](/workspace-administration/settings/organization/org-profile#managing-personas). Personas do not grant access to items or change existing permissions.
