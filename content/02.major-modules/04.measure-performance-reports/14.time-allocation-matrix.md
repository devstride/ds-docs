...
description: "Roll recorded time up through your work-item hierarchy or by job role, month by month, and export it to Excel."
---
...
With **Report rows → Job role**, each row is a recorded job role, such as Engineer or QA Analyst. Monthly hours, monthly shares, total hours, and allocation show how the same scoped time divides between roles. **Unspecified** includes older time without a recorded role; retired roles remain labeled. Each recorded hour appears once.
...
Choose **Item hierarchy** or **Job role** in **Report rows**. Configure the hierarchy below in either case: in the job-role view it chooses which items contribute time, while the rows themselves show role totals.
...
For **Job role**, the workbook has an **Assignee role** column, monthly hours, **Total hours**, and **Allocation**. A **Monthly allocation** row under each role shows its monthly percentage shares. Retired roles and **Unspecified** remain included, followed by **Grand total**. It uses the same completed report as the screen.
...
**Which job role?** The role captured when the time was recorded. Later assignment changes, member-role reordering, and role retirement do not reclassify completed time. Switching from hierarchy rows to job-role rows changes the breakdown, not the total for the same scope and dates.
