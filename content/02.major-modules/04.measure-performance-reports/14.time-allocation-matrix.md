...
description: "Roll recorded time up through your work-item hierarchy or by persona, month by month, and export it to Excel."
---
...
With **Report rows → Item hierarchy**, rows are your item hierarchy, one level per item type you configured, expandable and collapsible. Every parent branch also gets a **subtotal** row carrying its rolled-up hours and an **allocation** row expressing the same branch as percentages. A **Grand total** row stays pinned beneath the header as you scroll.

With **Report rows → Persona**, each row is a recorded persona, such as Engineer or QA Analyst. Monthly hours, monthly shares, total hours, and allocation show how the same scoped time divides between personas. **Unspecified** includes older time without a recorded persona; retired personas remain labeled. Each recorded hour appears once.
...
Choose **Item hierarchy** or **Persona** in **Report rows**. Configure the hierarchy below in either case: in the persona view it chooses which items contribute time, while the rows themselves show persona totals.
...
Saved views store the **configuration**, not the results — dates, row grouping, levels, item types, selections. Opening one drops you on the setup form with everything filled in; you press **Run report** for current numbers. That is what makes a monthly report repeatable. You can share a view by copying its link.
...
A single **Allocation Matrix** worksheet. For **Item hierarchy**, it contains:
...
For **Persona**, the workbook has an **Persona** column, monthly hours, **Total hours**, and **Allocation**. A **Monthly allocation** row under each persona shows its monthly percentage shares. Retired personas and **Unspecified** remain included, followed by **Grand total**. It uses the same completed report as the screen.
...
Positive **manual subtask time** is included too, against its parent item. Its completion date must fall inside the reporting period in the organization's timezone; an undated manual total is excluded. The same item-access and scope checks apply. Both row groupings count these records, including parents whose only time comes from subtasks.
...
**How time gets in.** The report reads completed DevStride time entries and dated manual subtask totals. Record time through the time controls, the item drawer's time-tracking tab, a timer, or the API. Workstreams organize the report's scope; time belongs to the work beneath them.

**Which persona?** The persona captured when the time was recorded. Later assignment changes, member-persona reordering, and persona retirement do not reclassify completed time. Switching from hierarchy rows to persona rows changes the breakdown, not the total for the same scope and dates.
