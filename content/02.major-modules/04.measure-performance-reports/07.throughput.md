...
The Throughput report counts the *net* number of items that reach the Done status in each period. As work moves through your workflow, an item that enters the Done status is counted, and an item that moves back out of the Done status is subtracted. Because of this net model, an item that is completed, reopened, and then completed again is not counted multiple times — the moves cancel out so the item is reflected once.
...
Period boundaries are aligned to UTC so that completions are attributed to the correct day, week, or month. This prevents an item completed close to a day or month boundary from being counted in the wrong period for teams in time zones west of UTC. Weekly periods run **Monday to Sunday**.

The chart draws nothing into the future: its axis ends at the **current period**, shown as a partial period holding today's snapshot, so a range that has only just begun is not stretched across empty columns.

## Related Reports

* * *

* [Cycle Time](/major-modules/measure-performance-reports/cycle-time) — Measures how long items take to move through your workflow.

* [Velocity](/major-modules/measure-performance-reports/velocity) — A related completion-based measure of delivered effort.
