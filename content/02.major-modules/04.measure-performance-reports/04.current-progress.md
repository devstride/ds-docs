...
* **Assigned** — items that have a person or job role versus those with neither.
...
## Open workload by job role

When your organization has job roles, **Open workload by assignee role** shows open item counts, work waiting for a person, planned estimates, and point effort for each role.

Work is counted under its chosen job role, or under the person's first active job role when no role is chosen. Work with neither is **Unspecified**. A person with several roles is not counted once under every role, and subtask estimates already included in their parent are counted once.

This describes current work and planned effort. It does not introduce availability budgets, cost rates, or a percentage split of a person's capacity. For time already recorded, use [User Time Tracking](/major-modules/measure-performance-reports/user-time-completed) or the [Time Allocation Matrix](/major-modules/measure-performance-reports/time-allocation-matrix), which retain the role captured with that time.
