...
* **Assigned** — items that have a person or persona versus those with neither.
...
## Open workload by persona

When your organization has personas, **Open workload by persona** shows open item counts, work waiting for a person, planned estimates, and point effort for each persona.

Work is counted under its chosen persona, or under the person's first active persona when no persona is chosen. Work with neither is **Unspecified**. A person with several personas is not counted once under every persona, and subtask estimates already included in their parent are counted once.

This describes current work and planned effort. It does not introduce availability budgets, cost rates, or a percentage split of a person's capacity. For time already recorded, use [User Time Tracking](/major-modules/measure-performance-reports/user-time-completed) or the [Time Allocation Matrix](/major-modules/measure-performance-reports/time-allocation-matrix), which retain the persona captured with that time.
