...
* **Tag** — filter by custom labels or tags for quick access to categorized items, such as `bug`. An item matches when it carries **any** of the selected tags, and the filter applies to every report and to the Items tab beneath it.
...
The **Items** tab groups items by the relevant period (day, week, or month, depending on your selected range). Weekly periods run **Monday to Sunday**, matching the chart. Each section header shows the period name along with a count rendered as **N items**, so you can gauge volume without expanding every group. Items load as you scroll, and the columns shown depend on the report — for example, Throughput shows **Item**, **Completed**, **Point estimate**, **Time estimate**, and **Hrs Spent**.
