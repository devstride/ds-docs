...
::alert{type="note" title="Your organization can restrict this to leaf items"}
An admin can turn off **Allow Parent Item Time Entries** under **Settings → Organization → Time Tracking**. When it is off, new time can only be logged against items whose work type has no child types: on an Epic, Feature, or other parent-type item the **New Entry** button is disabled with an explanation, and the API refuses the entry. Time already logged on parent items stays visible and editable. It is on by default. See [Allow Parent Item Time Entries](/workspace-administration/settings/organization/time-tracking-lock#allow-parent-item-time-entries).
::
...
The assignee here is primarily used for understanding accountability for the parent level, as is often done with organizations using RACI (responsibility) charts. If the owner of the parent does hands-on work at the parent level — coordination, review, integration — that time can now be logged directly on the parent item itself, where it shows as the parent's own time alongside the rolled-up child totals.
...
- On any work item, click the **Time Spent** field to log time directly against that item.
- On a **parent item**, the field's caption breaks the total down — how much is the item's own logged time and how much rolled up from child items — so the two are never conflated.
