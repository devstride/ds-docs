...
**Reusable description templates (Admins)**

Description templates are saved at the organization level, so the whole team can reuse them. Anyone with edit access can pick a template and generate a description, but **only Admins can save, edit, or delete reusable templates**.

If you are an Admin, the **Select a description template** row shows a **Save** link (or **Edit** when a template is already selected) that opens a **Manage Template** section. There you can enter a **Label** and a **Description** and then **Save template** (or, when editing an existing one, **Save as new** or **Update**); a trash control deletes the selected template. The Label is limited to 100 characters and the template Description to 200 characters.

<!-- TODO: screenshot — "Manage Template" section visible to Admins, with Label, Description, and Save controls -->
