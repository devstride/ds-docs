...
3. **The delivery loop** — shipping code with `/ds-build-item` and its supporting skills.
...
- **Shipping a story that's already planned** — picking up the next unblocked item and carrying it through branch, build, review, PR, and merge — start with **[The Delivery Loop](/developer-docs/claude-skills-delivery-loop)** and run `/ds-build-item <item|next>` (drive it under `/loop` to walk a whole plan).
- **Handling a one-off ticket or bug that isn't in any plan** — an inbound support request or ad-hoc fix — run `/ds-create-story` or `/ds-create-defect`. They create the item, place it, assign it to you, and then deliver it through the same build loop **once** (single-shot, no plan walk). See **[The Delivery Loop](/developer-docs/claude-skills-delivery-loop)**.
...
- [The Delivery Loop](/developer-docs/claude-skills-delivery-loop) — `/ds-build-item` and its supporting skills for shipping code
