...
Findings and deferrals that are real but have **no existing tracked item to absorb them** are not left as prose — they're collected into an **untracked-deferral list** that `/ds-build-item` step 6.5 turns into real, dependency-spliced DevStride items via `/ds-insert-defect` / `/ds-insert-story`. This is the "mid-build escape hatch" for newly discovered work; see [The Planning Loop](/developer-docs/claude-skills-planning-loop) for how those two skills splice an item into the graph.
...
- [The Planning Loop](/developer-docs/claude-skills-planning-loop) — `/ds-plan`, `/ds-insert-story`, `/ds-insert-defect`, and `/ds-comprehend-plan`: how work gets onto the dependency graph that `/ds-build-item` selects from
