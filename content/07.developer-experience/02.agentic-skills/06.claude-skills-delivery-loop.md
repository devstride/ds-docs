...
8. once the deploy is confirmed, run your local post-deploy health skill (`release.postDeployCheckSkill`) before anything public is written: `PASS` continues, `FAIL` stops for a rollback decision, `NOT RUN` asks you whether to proceed, and an unset key reports **not configured**;
