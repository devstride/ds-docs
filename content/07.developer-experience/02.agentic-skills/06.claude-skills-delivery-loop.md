...
Explicit configuration wins over the profile. `epicIntegrationBranches.autoRelease`, `epicIntegrationBranches.fastStoryMerges.enabled`, `review.pollTimeoutMinutes`, and the three `review.*` CI-ordering flags stand whenever they are present in the file; the profile supplies a value only when the key is absent. A contradiction is honored and reported, never silently resolved.
