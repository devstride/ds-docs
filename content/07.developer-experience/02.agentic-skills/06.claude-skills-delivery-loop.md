...
1. fetch, read what other authors landed on both branches recently, and say so when another session appears active; then compute the release delta, and settle the release source first — every ready pull request into it is merged (it joins the release) or parked, because a merge beneath a ready release pull request re-runs its CI and stales the reviewed diff (`ci.freezeBaseWhileReleasePrReady`);
...
9. after the deploy is verified live and healthy, write a release note through your local release-notes skill — **only** if you passed `--release-notes`.
...
Run `/clear` before moving from setup, planning or Doctor into building, reviewing or releasing. The session gate introduced in 3.5.0 keeps those job classes separate; a build loop may still deliver multiple stories in one session. See [Session boundaries and deferred defects](/developer-experience/agentic-skills/configuration-reference#session-boundaries-and-deferred-defects) for intentional overrides.

Verified defects below the profile's fix floor are filed beside the plan in its **Deferred defects** container and related to the reviewed item. They do not enter the active dependency chain, are not auto-selected, and do not hold the completed release unit open. Newly discovered implementation scope still enters the plan through `insert-story`.
