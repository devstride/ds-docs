...
1. a profile passed explicitly to the invoking skill;
...
- the configured local CLI reviewer, when present and when the profile runs it on items, through `review`’s local-only mode, for at most the profile’s number of rounds;
