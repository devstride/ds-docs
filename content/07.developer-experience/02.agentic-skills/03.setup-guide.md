...
`/devstride:setup validate` proves the config file. `/devstride:doctor` checks the broader machine and repository state, including the installed plugin version, marketplace registration, DevStride connection, CI draft gate, and merge gates. Doctor diagnoses read-only, then offers the repairs that are safe to automate and re-verifies each; it never repairs a workflow file, git state, a DevStride record, or a deployed stage.
