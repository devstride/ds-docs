...
description: "The .claude/ds-config.json settings read or written by DevStride for Claude Code v0.8.1."
---

# Configuration Reference: `.claude/ds-config.json`

This page describes the repository contract for plugin **v0.8.1**. The file lives in the consuming repository, and its values override the plugin’s inline fallbacks.
...
| Key | Shape | Default | Purpose |
|---|---|---|---|
| `hierarchyRoles.releaseUnit` | string or `null` | Resolved at runtime | Container type whose completion cuts a release. |
