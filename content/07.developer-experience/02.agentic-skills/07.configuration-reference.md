...
| `profileOverrides` | object of knob → value | absent | Pins individual profile knobs for every profile, for example `{"maxLocalReviewRounds": 2, "reviewBreadthCeiling": "HIGH-RISK", "verificationGrouping": "per-finding"}` — the last restores one verifier per finding at high-risk breadth, where the default verifies findings one agent per file-group, security findings always singly. Keys are the knob names from the profile contract and values use its vocabulary. An override cannot go below a floor, and an unknown knob name is reported and ignored rather than honored as something else. |
...
| Key | Shape | Default | Purpose |
|---|---|---|---|
| `review.localCommand` | string or `null` | `null` | Optional second local review engine. `null` means the built-in Claude pass is the only local reviewer. Naming it does not schedule it; see [Delivery profile](#delivery-profile) for where and how often it runs. The template carries a `<base>` placeholder (the ref the engine diffs against) and may carry `<context>` (a re-review round then feeds a distilled account of the previous round on stdin; the first round removes the token). A template with neither placeholder keeps working — `--base` is appended. |
| `review.adaptiveReviewerWait` | boolean | absent (on) | Absent means on: the wait for a registered cloud reviewer is bounded by that reviewer's learned latency (its 95th percentile plus a margin, per machine, from GitHub's timestamps) instead of the full `pollTimeoutMinutes`. `false` pins the fixed bound. The learning cache is advisory and lives under `~/.cache/devstride-plugin/`; delete it to start cold. |
| `review.localReReviewScope` | `"delta"` or `"full"` | absent (`"delta"`) | A local re-review round reads only the fixes since the previous round, falling back to the whole diff by a computed rule (a new file, more than half the earlier round's lines, or a rebase). `"full"` pins the whole-diff re-review of earlier releases. |
