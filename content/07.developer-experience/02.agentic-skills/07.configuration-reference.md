...
description: "The .claude/ds-config.json settings read or written by DevStride for Claude Code v3.5.1."
---

# Configuration Reference: `.claude/ds-config.json`

This page describes the repository contract for plugin **v3.5.1**. The file lives in the consuming repository, and its values override the plugin’s inline fallbacks.
...
| `epicIntegrationBranches.autoRelease` | boolean or `"ask"` | `false` for `standard` and `enterprise`; `true` for `prototype`, which setup says out loud | When `true`, the loop cuts and merges the release-unit pull request after the final leaf. `false` stops at release-ready. `"ask"` requests approval for each completed release unit before cutting and merging its pull request. A present value wins over the profile. |
...
## Session boundaries and deferred defects
...
| `defects.deferredContainerTitle` | string | `"Deferred defects"` | Names the container directly under the plan root where below-floor review defects are filed. |
