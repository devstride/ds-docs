...
2. **[The skill audit](/developer-experience/agentic-skills/skill-audit)** — a flat reference of every `/ds-*` skill: how you invoke it, its arguments, and what it does.
...
Every skill in this system is prefixed `ds-` (short for "DevStride") and invoked as a slash command: `/ds-build-item`, `/ds-plan`, `/ds-pr`, and so on. The prefix signals "this skill knows DevStride's specific conventions" — branch naming, item statuses, the completion ritual — as opposed to a generic Claude Code skill.
::
...
Most `/ds-*` skills — and especially the top-level delivery orchestrator, `/ds-build-item` — are written to run **end-to-end without pausing**. `/ds-build-item` in particular is explicitly designed to be driven under `/loop`: select a story, mark it In Progress, branch, build, review, open a PR, address findings, merge, run the completion ritual, sync, and move to the next story — all without a confirmation prompt between steps. Stories under an Epic batch onto that epic's own **integration branch** (with the slow golden-dataset gates skipped per story) in **fast develop mode** — no PR of their own, just both local review engines plus green local suites, then a local merge; when the epic's last story lands, the loop automatically cuts the epic-branch → `develop` release PR and drives it through the full review stack before merging, so `develop` only ever receives whole, reviewed epics. Items with no epic ship straight to `develop` and always keep their own full-ritual PR, since no epic release follows them. **Any PR the loop opens opens as a draft, so the review engines settle before any CI runs** — CI then runs once, on the final reviewed diff. `/ds-plan` is the one major exception to this pattern — see the callout below.
...
- **Building out a whole new hierarchy** — turning a Module/Capability/Epic into a real, dependency-wired Capability → Epic → Story hierarchy through the interactive discovery loop — start with **[The Planning Loop](/developer-experience/agentic-skills/claude-skills-planning-loop)** and run `/ds-plan <item>`.
- **Adding a single Story or Defect to a plan that already exists** — don't use `/ds-plan` for this; go straight to **[The Planning Loop](/developer-experience/agentic-skills/claude-skills-planning-loop)** and run `/ds-insert-story` or `/ds-insert-defect` directly. `/ds-plan` is reserved for building/extending a whole hierarchy.
- **Shipping a story that's already planned** — picking up the next unblocked item and carrying it through branch, build, review, PR, and merge — start with **[The Delivery Loop](/developer-experience/agentic-skills/claude-skills-delivery-loop)** and run `/ds-build-item <item|next>` (drive it under `/loop` to walk a whole plan).
- **Handling a one-off ticket or bug that isn't in any plan** — an inbound support request or ad-hoc fix — run `/ds-create-story` or `/ds-create-defect`. They create the item, place it, assign it to you, and then deliver it through the same build loop **once** (single-shot, no plan walk). See **[The Delivery Loop](/developer-experience/agentic-skills/claude-skills-delivery-loop)**.
...
- [The Planning Loop](/developer-experience/agentic-skills/claude-skills-planning-loop) — `/ds-plan` and its supporting skills for shaping roadmap work
- [The Delivery Loop](/developer-experience/agentic-skills/claude-skills-delivery-loop) — `/ds-build-item` and its supporting skills for shipping code
- [Local Development](/developer-experience/intro/local-development) — the underlying `ds` CLI these skills drive under the hood
