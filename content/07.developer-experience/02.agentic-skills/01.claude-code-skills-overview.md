...
Some floors hold under every profile: one maximum-effort adversarial Claude pass always runs, the security lens is mandatory when a diff touches an auth boundary (and a security finding is always verified by its own agent, never batched), a fast-merged item has at least one review engine behind it, an item’s local gate must be green before it merges, and the release pull request still gets every configured cloud reviewer and CI over the full diff.
