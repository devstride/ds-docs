...
- **What it does:** the **orchestrator**. It runs the whole loop — select → mark In Progress → branch → build → adversarial review → PR → merge → completion ritual → sync → next — composing the skills below and owning only the DevStride glue (next-item selection, status transitions, the completion ritual). Designed to run under `/loop` for full autonomy, pausing only at a genuine fork. It auto-detects **plan-loop mode** (walk a sequenced plan) vs **one-off single-shot mode** (ship one standalone item once and terminate), and supports an opt-in integration-branch working base for large initiatives.
