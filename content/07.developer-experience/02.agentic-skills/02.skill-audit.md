...
| [`/ds-build-item`](#ds-build-item) | `<item \| next \| under I####>` | **Orchestrator** — take one item from selection to merged-and-Done; batches an epic's stories onto its integration branch and auto-releases the completed epic to develop |
...
Two skills, each usable standalone. PRs open as **drafts**, which holds every CI job until all three review engines have settled; the ready-flip inside `/ds-review` is what releases CI.
...
- **What it does:** owns PR **creation** and **linking**. It opens the PR with the required **four-section description** (Simple Description · Technical Description · Notable Changes to System Architecture or Behavior · Testing Steps) — no `--fill`, and no AI-attribution text in the body — then delegates all review and CI gating to `/ds-review`. It opens the PR **as a draft** and requests the configured cloud reviewer(s) in the same call, so the cloud review overlaps the local engines. Standalone it asks the base branch and offers to link the item; driven by `/ds-build-item` the base is pre-answered (the epic integration branch, or develop) and linking is left to the loop. It also has an **epic-release mode** (head = epic branch, base = develop, body leads with the epic + its story list, every applicable gate mandatory, no `--delete-branch`).
