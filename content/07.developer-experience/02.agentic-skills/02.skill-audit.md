...
| [`/ds-build-item`](#ds-build-item) | `<item \| next \| under I####>` | **Orchestrator** — take one item from selection to merged-and-Done; batches an epic's stories onto its integration branch in fast develop mode (no per-story PR) and auto-releases the completed epic to develop |
...
- **Invoke:** `/ds-build-item <item | next | under I####>` · **writes live data (status moves, links)**
...
- **What it does:** the **orchestrator**. It runs the whole loop — select → mark In Progress → branch → build → adversarial review → PR → merge → completion ritual → sync → next — composing the skills below and owning only the DevStride glue (next-item selection, status transitions, the completion ritual, the epic release). Designed to run under `/loop` for full autonomy, pausing only at a genuine fork. It auto-detects **plan-loop mode** (walk a sequenced plan) vs **one-off single-shot mode** (ship one standalone item once and terminate). **Epic batching is the default:** a story under an Epic works on that epic's integration branch (`<prefix>/<MM-DD-YY>/<epic-number>-<epic-slug>`, created on first use, reused after) with the golden gates skipped per story. Such a story runs in **fast develop mode** — no PR of its own: both local engines (the build-time Claude pass + the Codex CLI at `xhigh`, via `/ds-review`'s local-only mode) plus green local suites, then a local `--no-ff` merge onto the epic branch, with Copilot and CI deferred to the epic release. When the epic's last story merges (and `autoRelease` is on), the loop auto-cuts the epic-branch → develop release PR, drives it through the full review stack — **a full-diff review, since under fast mode this is the first cloud pass over any of that code** — with any golden gate whose trigger fires mandatory, merges, and deletes the branch if configured to. Items with no Epic ship straight to develop and always keep the full per-story PR ritual (nothing follows them, so that PR is their only cloud gate), evaluated against the same three golden cases; `develop → master` stays owner-cut, and *that* base runs the full golden suite unconditionally.
...
- **What it does:** the everyday branch-creation utility. It aborts on a dirty tree, syncs the working base (the epic's integration branch when `/ds-build-item` passes one — the default for planned stories — else `develop`), creates a branch named `<user-prefix>/<MM-DD-YY>/<suffix>`, and pushes it with `-u`. Used both standalone and as `/ds-build-item` step 2.
...
- **What it does:** the **build engine** `/ds-build-item` invokes once a branch exists. Three phases: **UNDERSTAND** (for a substantive story, an ultracode Workflow of parallel reader agents → a concrete build plan + buildable-now-vs-deferred scope), **BUILD** (small increment → checks green → commit, repeated, with generated artifacts committed separately; the `verify.skipDuringStoryBuilds` suites — the slow golden dataset suite — are not run per story, and under the three-case rule an ordinary story matches none of them), and **ADVERSARIAL REVIEW** (a Workflow of parallel finders → per-finding verify → fix CONFIRMED/PLAUSIBLE, drop REFUTED). It opens no PR and merges nothing — it hands back a summary, deviations list, and untracked-deferral list.

### The review stack: `/ds-pr` → `/ds-review`

Two skills, each usable standalone. Note that an epic-branch story under **fast develop mode** reaches neither of them — it settles locally through `/ds-review`'s local-only mode, and the epic's release PR is where `/ds-pr` next runs. Any PR that *is* opened opens as a **draft**, which holds every CI job until the review engines have settled; the ready-flip inside `/ds-review` is what releases CI.
...
- **What it does:** owns PR **creation** and **linking**. It opens the PR with the required **four-section description** (Simple Description · Technical Description · Notable Changes to System Architecture or Behavior · Testing Steps) — no `--fill`, and no AI-attribution text in the body — then delegates all review and CI gating to `/ds-review`. It opens the PR **as a draft** and requests the configured cloud reviewer(s) in the same call, so the cloud review overlaps the local engines. Standalone it asks the base branch and offers to link the item; driven by `/ds-build-item` the base is pre-answered (develop for a one-off, or the epic branch for an epic release) and linking is left to the loop. It also has an **epic-release mode** (head = epic branch, base = develop, body leads with the epic + its story list, every applicable gate mandatory, no `--delete-branch`).

#### `/ds-review`

- **Invoke:** `/ds-review [PR#]` · git + GitHub
- **Argument:** a PR number (e.g. `2667`); empty → the current branch's open PR.
- **What it does:** the review + CI-gating **engine**. It launches all three engines concurrently — **Claude** (the build-time adversarial pass, or run here on any path that didn't have one), **Codex** (the local CLI at its max reasoning tier — never `@codex review`, whose GitHub connector is disabled), and **Copilot** (cloud, requested via GraphQL) — polls **reviewers only** (no CI is running: the PR is a draft), collects **both halves** of a cloud review (inline threads *and* the review body, including its suppressed-findings block), **verifies** every finding against the real code, triages each into fix / dismiss-with-rationale / capture-out-of-scope / ask-on-fork, applies fixes via `/ds-push`, **replies to and resolves** every addressed thread, then rebases, marks the PR **ready** — releasing CI — and settles it green. It also owns the slow-gate policy: the golden suite runs in exactly three cases (the diff touches that code directly · the PR promotes to `master` · a human adds the `run-golden` label), and is otherwise omitted, with its absent check expected rather than red. Standalone it notifies when the PR is ready; driven, it hands its findings back to the caller. A third **local-only** mode serves fast develop mode: given a base *ref* rather than a PR, it runs the Codex stream alone and stops after triage — no cloud reviewer, no threads, no ready-flip, no CI — and there an unavailable Codex is a hard stop, since nothing downstream would catch what it missed.

  The detailed GitHub-API procedures and the slow-gate applicability computation live in `ds-review/references/` — loaded only when a step needs them, so the always-loaded instructions stay small.
