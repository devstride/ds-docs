...
| Skill | Invocation | What it does |
|---|---|---|
| `setup` | `/devstride:setup [validate | ecosystem | verify | ci-inspect | branches | engines | docs | ci]` | Detects repository settings, confirms DevStride hierarchy roles, writes only `.claude/ds-config.json`, and validates it. `validate` and detector-only runs write nothing. |
| `doctor` | `/devstride:doctor [env | plugin | devstride | config | ci | gates | docs]` | Inspects the installation, connection, config, effective branch roles, CI ordering, and merge gates. It suggests existing remote branches when static fallbacks do not resolve. Strictly read-only; prints fixes but never runs them. |
| `ci-audit` | `/devstride:ci-audit [days] [workflow]` | Measures what CI actually costs over a window: executed runs per pull request (the design is one), post-merge push minutes, release pull requests re-run by a moving base, and the named offenders with their likely cause. Read-only. |
