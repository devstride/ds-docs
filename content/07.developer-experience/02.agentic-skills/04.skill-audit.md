---
title: "Skill Audit"
description: "A compact reference for all seventeen skills shipped by the DevStride Claude Code plugin."
---

# Skill Audit: All Seventeen Skills

Version **0.8.1** ships the seventeen skills below. They always invoke as `/devstride:<name>`, including when the plugin was installed through the shorter `ds@devstride` marketplace entry.

## Planning

| Skill | Invocation | What it does |
|---|---|---|
| `plan` | `/devstride:plan <parent-item> [description]` | Interactively shapes a container → release-unit → leaf roadmap, writes approved items and dependencies, rationalizes dates, and numbers the execution order. Reads the existing plan first. |
...
| `doctor` | `/devstride:doctor [env | plugin | devstride | config | ci | gates]` | Inspects the installation, connection, config, CI ordering, and merge gates. Strictly read-only; prints fixes but never runs them. |
