...
`/ds-build-item <item|next>` runs the whole loop: **select → mark In Progress → branch → build → adversarial review → PR → merge → completion ritual → sync develop → next**. It is the ORCHESTRATOR — it composes the other skills below by name and owns only the DevStride-specific glue (next-story selection, status transitions, the completion ritual). It is explicitly designed to run under `/loop` for full autonomy.
