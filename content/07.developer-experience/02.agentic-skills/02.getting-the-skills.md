...
The current published version is **3.1.1**, with **twenty** `/devstride:*` skills.
...
From 3.1.0 the simplest way to update is:

```
/devstride:update
```

It updates the exact installation that loaded the command — preserving whether you installed as `devstride` or `ds`, and at user or project scope — verifies the installed files against the published tag, and then tells you whether `/reload-plugins` is enough or Claude Code needs a restart. It stops rather than guessing when an install is pinned, administrator-managed, or ambiguous.

The plugin also checks for a newer release every time a session starts and tells you when you are behind — silently when you are current or offline, and with one line naming the exact fix when you are not. A repository can have it apply the update at session start (`plugin.autoUpdate` in `.claude/ds-config.json`, which ships enabled) or hold a version deliberately (`plugin.pin`). The automatic path only ever updates an install scoped to that repository; a shared user-scope copy is handed to `/devstride:update` instead, and an administrator-managed one is left alone. See the [configuration reference](/developer-experience/agentic-skills/configuration-reference) for the block.
