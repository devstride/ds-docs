...
description: "Use DevStride’s planning skills to turn a container into a dependency-ordered roadmap the delivery loop can execute."
---
...
Pass an existing DevStride container. The skill reads what is already below it, asks about the plan, shows the proposed hierarchy and specs, and creates nothing until you approve them.

## What a finished plan contains

- Your organization’s real container, release-unit, and leaf work types.
...
3. **Draft after the shape is approved.** Parallel agents may draft container, release-unit, and leaf descriptions, but they do not make unresolved architecture or scope decisions and do not write DevStride data.
...
- re-run the selector and tell you if an earlier-dated container still wins.
...
The result is a synthetic critical-path view: each leaf is one day, dependencies precede dependents, and containers span their descendants. It is not a delivery forecast.
...
- **Repoint** — real intent, but attached to a container or the wrong leaf;
