...
It's easy to assume `.github/workflows/` is where deploys live, since that's the conventional place for them. In this repo, it isn't. All seven workflow files run checks or maintenance jobs — **none of them deploy anything**. Most run on pull requests, and several also run on pushes to `develop` and `master`:
...
Everywhere else the required **Backend Tests** check goes green with its reason printed, and no shard runs. A green Backend Tests check on a pull request into `develop` is therefore **not** evidence the suite ran: read the aggregate job's log line. The trade-off is deliberate — a merge that breaks the suite is found when the release is cut rather than minutes after the merge, because every story is run against the full suite locally before it merges.
