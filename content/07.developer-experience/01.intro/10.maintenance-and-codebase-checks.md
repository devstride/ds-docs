...
description: "Codebase wiring audits, event-system delivery and topology checks, data cleanup and repair scripts, operational toggles, and historical one-off migrations available via ds script."
---
...
This page covers everything under `ds script` that isn't already documented on the [Local Development](/developer-experience/intro/local-development), [API Development](/developer-experience/intro/api-development), or [Stripe Integration](/developer-experience/intro/stripe-integration) pages (API client/docs generation, `set-config`, and `reset-db` live there instead).
...
- `delete-orphan-stacks` also runs internally at the start of every `ds run backend` and inside `ds script reset-db` on AWS-bound stages (the local Docker reset skips it).
...
::alert{type="warning" title="Deployed stages only"}
`event-contract-test` must run against a **deployed** stage, not a personal `sst dev` session — under live-lambda dev the assets-module event queue doesn't exist and the consumer transports are swapped, so the assertions can't pass. Run it against a real stage (and mind the `--allow-prod-alarms` guard before pointing the negative path at `prod`/`dev`).
::
...
- [Local Development](/developer-experience/intro/local-development) — Running servers, code generation, and data commands
- [API Development](/developer-experience/intro/api-development) — Generating the typed API client and OpenAPI docs
