...
::alert{type="note" title="The migration-drift guard"}
Both `ds golden push` and `ds golden import` refuse to run when the checkout, the golden source, and
...
error names the migrations actually missing on each side rather than a position in the list.
::
...
::alert{type="danger" title="Blocked on prod and shared staging"}
Throws immediately if `DEVSTRIDE_STAGE === "prod"`; the golden stage guard also refuses the shared `dev` staging stage. A bare `ds script reset-db` binds to AWS — on the local stack always go through `ds worktree cli <instance>`, using the instance registered for the checkout you are on.
::
...
Defined in `cli/commands/stripe.ts`. Three subcommands: provisioning Stripe products, a read-only billing inventory export, and a test-mode proof of the one-off invoice flow. See [Stripe Integration](/developer-experience/intro/stripe-integration) for what each one does.
...
Creates Stripe products/prices via `CreateProductCommand`. No flags. Not idempotent — run it once per Stripe account.
...
Needs `STRIPE_INVENTORY_KEY`, a **restricted** read-only key (`rk_…`); a full `sk_…` secret key is refused.
...
Stripe TEST-MODE proofs for one-off invoices and subscription cancellation. No flags. Needs `STRIPE_PROOF_KEY`, a test-mode key.

::alert{type="note" title="Commands that do not exist"}
There is no `ds stripe sync-products`, `sync-customers`, `check-quantities`, or any Slack `backfill-notifications` command. **`create-customers` and `find-subscription-quantity` were removed** — `billing-inventory` replaces the latter.
::
