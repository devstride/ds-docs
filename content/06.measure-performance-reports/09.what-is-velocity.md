...
::alert{type="tip" title="Pick the lane that matches your definition of done"}
If your team only considers work complete once it ships, set the Status selector to the lane you ship from (for example, **Released** or **Deployed**) rather than the default **Done** lane. The chart, the per-cycle completed counts, and the Items tab all update to match.
::
