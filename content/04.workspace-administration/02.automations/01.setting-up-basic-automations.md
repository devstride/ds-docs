...
Most field-change actions on items (changing status, priority, team, and so on) are available only when the trigger is *Work Item Created*, an item-updated trigger, or *Due Date Arrived*. *Complete Child Items or Subtasks* is available for item-updated triggers, such as *Status Changed*. Workstream update actions apply only to workstream-created and workstream-updated triggers.
::
...
### Work with job roles in Automations

Use **Assignee Role Updated** to react when an item's chosen job role changes, **Assignee Role** conditions to narrow the rule, and **Change Assignee Role** to assign the required kind of work. Role values can also be used in supported automation references.

These changes follow the same assignment rules as the chooser: selecting a role preserves a compatible person and clears an incompatible one. Clearing the role while keeping a person with job roles applies that person's first active role. Job roles grant no extra permissions to the automation or to members.
