...
* **Messaging** — *Send Notification*. When the notification goes to Slack or Microsoft Teams, any value it fills in from the item or trigger — a title, a custom field, a ticket subject from a customer's email — appears as plain text, so it can never become a clickable link or an @channel / @here mention. The wording and formatting you write yourself in the notification are sent as written.
...
Most field-change actions on items (changing status, priority, team, and so on) are available only when the trigger is *Work Item Created*, an item-updated trigger, or *Due Date Arrived*. *Complete Child Items or Subtasks* is available for item-updated triggers, such as *Status Changed*. The *Subtask Created* trigger offers only *Change Subtask Assignee*, *Change Subtask Status*, *Set Subtask Due Date*, *Send Notification*, and *Call Webhook*, and those three subtask actions appear only under it. Workstream update actions apply only to workstream-created and workstream-updated triggers.
::
...
For the separate task of removing an archived board or folder, see [permanent board and folder deletion](/major-modules/boards/working-group-folders#permanently-deleting-an-archived-board-or-folder).
