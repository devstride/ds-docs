...
::alert{type="note" title="Filtered lists show only what you can read"}
These filters match on the **items inside** a workstream, so a workstream appears only when an item you have access to matches. If your access is restricted, a filtered list can be shorter than a colleague's — see [What you see in filtered lists](/major-modules/workstreams-and-work-items/workstream-and-item-permissions#what-you-see-in-filtered-lists).
::
