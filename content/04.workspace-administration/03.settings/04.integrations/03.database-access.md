...
Managing Database Access requires the **Manage database credentials** permission, which **Owner** and **Admin** both have by default. You can also grant it to a custom role. Every operation is enforced on the server, and the Organization Settings area itself isn't available to Members—so they never see the Database Access menu.
::
...
2. In the **Integrations** section of the left settings rail, select **Database Access** (the database icon, listed alongside GitHub, Slack, and Identity Providers).
...
<!-- TODO: screenshot — Database Access menu item in the Integrations section of Organization Settings -->
...
The password is stored encrypted (AES-256-CBC) and can be re-revealed at any time through **View Details** while the credential is **Active**, by anyone whose role can manage database credentials (see below). You don't need to capture it on first sight—but treat it like any other secret and only share it with people who should have access.
::
...
- **Restricted to roles that manage database credentials.** Generating, viewing, regenerating, revoking, activating, syncing, and deleting credentials all require the **Manage database credentials** permission. **Owner** and **Admin** both hold it by default, and you can grant it to a custom role in [Configure Settings → Permissions](/permissions/roles-and-permissions).

## Related pages

- [GitHub](/settings/integrations/github)

## Upcoming table renames

DevStride is renaming its status and workstream vocabulary throughout the product — the API and MCP tools already use the new words, with deprecation windows listed on the [API keys](/personal-workspace/my-account/api-keys) page — and the read-only database will follow in a later release: the tables `lanes`, `lane_collections` and `folders` become `statuses`, `status_collections` and `workstreams`, and the kept tables gain canonical twins of their `lane_*` and `*_folder_*` columns. Nothing changes for your queries on the day of that release: the current table names stay available as read-only views with identical columns, and the current column names keep working alongside their twins.

The legacy names are removed no earlier than **90 days after the 2026-09-07 notice** sent to every organization with active credentials — so not before **2026-12-06** — and only in a release after the rename itself. If you maintain dashboards or scheduled queries against these tables, plan to move them to the new names during that window.
