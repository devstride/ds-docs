...
- **Restricted to roles that manage database credentials.** Generating, viewing, regenerating, revoking, activating, syncing, and deleting credentials all require the **Manage database credentials** permission. By default only the **Owner** holds it — it isn't part of the standard Admin role and is granted to another role only by an Owner, in [Configure Settings → Permissions](/permissions/roles-and-permissions).
