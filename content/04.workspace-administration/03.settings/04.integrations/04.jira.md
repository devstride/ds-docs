...
To reach the integration, open **Settings** from the lower-left corner of the platform, then choose **Integrations** from the header. The list on the left includes **Credentials**, **GitHub**, **Slack**, **Microsoft Teams**, **Azure DevOps**, and **Jira**.
...
The webhooks DevStride registers when it imports a project carry a **per-registration secret** in
their callback URL, and every delivery is checked against it before anything is written — a POST
that does not present the right secret is refused and nothing in your organization changes. You
do not manage this secret; DevStride mints a fresh one each time it registers a project's webhooks,
and importing (or re-importing) a project rotates it. Projects imported before this check existed
keep syncing as they always have; re-import such a project to move it onto a verified registration.
...
Imported items are **read-only in DevStride** until two-way editing exists: you can open their relationships and blocker panels, follow the board link, and view everything, but adding or removing relationships, setting a blocker, and other edits stay blocked.
...
## If the connection's access is revoked

If the Jira credential's authorization expires, or someone revokes it in Jira, DevStride tells you so directly: you'll see a message asking you to **reconnect the integration to restore access**, rather than a generic failure. Reconnect the credential in **Settings → Integrations → Jira** and syncing resumes.

DevStride only remembers what Jira successfully returned, so once access is back it is picked up on the very next request. A moment of lost access no longer leaves Jira screens failing for hours afterwards. What DevStride remembers is also kept separately for each Jira site, so two sites never see each other's issue types or fields.
