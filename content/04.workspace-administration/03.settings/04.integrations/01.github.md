...
Each linked PR carries its current status (Open, Draft, Closed, or Merged) along with review and check information. Checks show one line per check with its latest result, as on the pull request's own GitHub page, so the list and the check count on the work-item card reflect where things stand now rather than every run ever reported. Linked commits show their subject line — the first line of the commit message — with a link to GitHub for the full details. The repository also appears in the pull-request detail header, which makes items spanning several codebases easier to scan.
...
If the attach fails, the dialog stays open with your URL still in the field, so you can correct it and try again without retyping. See [Troubleshooting](#troubleshooting) for what each message means.
...
::alert{type="tip" title="Merged pull requests work, even after the branch is deleted"}
You can attach a pull request that has already been merged, including one whose head branch GitHub deleted on merge — which many repositories do automatically. It still shows its original branch name on the pull request's own row, and its checks still appear. The branch itself won't be under **Linked Branches** — GitHub has deleted it, and that list only holds branches DevStride is still tracking.
::
...
- **GitHub could not process a lookup for `owner/repo#number` (HTTP 422), so it was not linked** — GitHub rejected part of the request. Check that the pull request and its latest commit still exist on GitHub. Previously this case surfaced as a generic server error.
- **GitHub returned a 5xx while fetching the pull request** — GitHub itself is having trouble. Try again in a moment.
