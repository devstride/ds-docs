...
Open **Settings → Organization → Integrations → Identity Providers → Add Identity Provider** and choose your protocol. A panel headed **Give these to your provider** appears, listing each value with a copy button, what it's for, and the other names your provider might use for it.
...
DevStride gives you **one** value; you bring back three.
...
Create an application (app registration) in your identity provider, set that redirect URI on it, then collect three values to bring back to DevStride — see [where these live in Entra and Okta](#where-these-values-live-in-your-identity-provider):
...
DevStride gives you **two** values; you bring back your provider's metadata.
...
Create the SAML application in your identity provider, paste both values in, then export your provider's **metadata** — either a metadata URL or the metadata XML document — to bring back to DevStride. [Where these live in Entra and Okta](#where-these-values-live-in-your-identity-provider).
...
DevStride can't show you these — they belong to your tenant. Here's where each one is.
...
**OIDC** — [Entra admin center](https://entra.microsoft.com) → **Identity → Applications → App registrations → New registration**. Microsoft's own walkthrough: [Register an application](https://learn.microsoft.com/entra/identity-platform/quickstart-register-app).

| DevStride asks for | Where it is in Entra |
| --- | --- |
| **Redirect URI** *(you paste ours in)* | On registration, choose platform **Web** and paste DevStride's Redirect URI. |
| **Client ID** | The app's **Overview** page → **Application (client) ID**. |
| **Issuer URL** | Build it from **Overview → Directory (tenant) ID**: `https://login.microsoftonline.com/<that-guid>/v2.0`. |
| **Client secret** | **Certificates & secrets → Client secrets → New client secret**. See [Add credentials](https://learn.microsoft.com/entra/identity-platform/how-to-add-credentials). |
...
**SAML** — [Entra admin center](https://entra.microsoft.com) → **Identity → Applications → Enterprise applications → New application → Create your own application**, then choose *Integrate any other application you don't find in the gallery*. Open **Single sign-on → SAML**. Microsoft's walkthrough: [Set up SSO for an application](https://learn.microsoft.com/entra/identity/enterprise-apps/add-application-portal-setup-sso).

| DevStride asks for | Where it is in Entra |
| --- | --- |
| **Identifier (Entity ID)** *(you paste ours in)* | **Basic SAML Configuration** → *Identifier*. Use DevStride's **SP Entity ID**. |
| **Reply URL** *(you paste ours in)* | **Basic SAML Configuration** → *Reply URL*. Use DevStride's **ACS URL** — see the caution above about Entra reusing this label. |
| **Metadata URL** | **SAML Certificates** → **App Federation Metadata Url**. |
...
You already have this dialog open from Step 1 — carry straight on to the fields.
...
| Sign-in fails at your identity provider, before ever reaching DevStride | Re-check the values your provider holds against the **Give these to your provider** panel in the DevStride dialog — the **Redirect URI** for OIDC, the **ACS URL** and **SP Entity ID** for SAML. These fail on your provider's side, so DevStride records nothing to help diagnose them. Copy rather than retype, and beware Entra labelling two different values *Reply URL*. |
