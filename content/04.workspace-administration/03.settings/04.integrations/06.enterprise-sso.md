...
4. [Confirm SSO works](#confirm-sso-works-before-you-tell-your-team)
5. [Turn on automatic member provisioning](#step-4-optional-turn-on-automatic-member-provisioning) (optional)
...
**SAML** — in the [Entra admin center](https://entra.microsoft.com), open **Entra ID → Enterprise apps** in the left sidebar, then:
...
4. In the app that opens, go to **Single sign-on** and choose **SAML**. Microsoft's walkthrough: [Set up SSO for an application](https://learn.microsoft.com/entra/identity/enterprise-apps/add-application-portal-setup-sso).
...
On lower Entra plans you'll see *"Groups are not available for assignment due to your Active Directory plan level"* — assign individual users instead. See [Assign users and groups](https://learn.microsoft.com/entra/identity/enterprise-apps/assign-user-or-group-access-portal).
::
...
::alert{type="danger" title="Most DNS providers want the name WITHOUT your domain on the end"}
GoDaddy, Cloudflare, Namecheap, Squarespace and most others append your domain to whatever you type in the *Name* field. So enter just:
...
::alert{type="tip" title="Use a private window — you're already signed in"}
You're signed in with a password right now, so the login page won't show you what a member sees. Open a **private / incognito window**, or sign out first. A private window is safer: if anything goes wrong you still have your normal session.
::
...
| *Your organization doesn't allow signing in with your identity provider* | Most often **Require multi-factor authentication** is on. See [SSO and multi-factor authentication](#sso-and-multi-factor-authentication). |

Only once your own sign-in works should you point the rest of your team at it — see [How your team signs in](#how-your-team-signs-in).
...
::alert{type="note" title="By default, SSO is an extra way in — not the only way"}
...
If you want SSO to be the **only** way in, that is a separate, deliberate switch: see [Requiring single sign-on](#requiring-single-sign-on).
::
...
::alert{type="warning" title="Requiring SSO is the setting most likely to lock people out"}
Before turning it on, confirm SSO actually works end to end for a real member — see [Confirm SSO works before you tell your team](#confirm-sso-works-before-you-tell-your-team). Members created through SSO have no password to fall back on.
...
- You cannot delete your **last** working identity provider while SSO is required — the deletion is refused, not merely discouraged. See [Removing an identity provider](#removing-an-identity-provider).
...
- **Members keep their accounts and their history.** An account that signed in through the old provider still carries a link to it, but that link is simply ignored — signing in through the new provider adds a second link alongside it and works normally. Verified by deleting a live provider and rebuilding it. (If a member genuinely can't sign in afterwards, see the troubleshooting table.)
...
If your organization **requires single sign-on**, DevStride refuses to delete your last working provider — that deletion would leave nobody with a way in. Turn off **Require single sign-on** first, or add a second working provider. See [Requiring single sign-on](#requiring-single-sign-on).
::
...
- **It does not force SSO by default.** Out of the box, members with a password can still use it. If you want SSO to be mandatory, turn on [Require single sign-on](#requiring-single-sign-on) — it is opt-in, and off until you choose it.
