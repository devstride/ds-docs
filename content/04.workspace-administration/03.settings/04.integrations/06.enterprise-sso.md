...
::alert{type="caution" title="Entra says \"Reply URL\" for two different things"}
In Microsoft Entra, the **OIDC** redirect URI and the **SAML** ACS URL are both labelled *Reply URL*, and they are not the same value. Match on what DevStride calls it, not on the label in the form — the dialog lists every alias for exactly this reason.
::
