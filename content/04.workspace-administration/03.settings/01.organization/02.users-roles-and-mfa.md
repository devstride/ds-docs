...
description: "Invite and manage organization members, understand the Owner, Admin, and Member roles, sign in with Google, edit a member from the Edit User dialog, reset a user's password or multi-factor authentication, and change your own password."
...
Members can sign in with their **Google account** instead of a password, using the **Sign in with Google** button on the login page. There is nothing to configure per member — DevStride matches the Google account to the member by **verified email address**:

- A member whose email matches their Google account's verified email can sign in with Google right away; their first Google sign-in links the two, and both sign-in methods keep working.
- An **invited** member can accept their invitation by signing in with Google — no password setup required. Their account is activated exactly as if they had completed the invitation email.
- A Google account that doesn't match any member is **refused**: the visitor is told the account isn't linked and to ask their administrator for an invitation. No account is ever created from an unmatched Google sign-in.

::alert{type="note" title="Organizations that require MFA"}
If your organization **requires multi-factor authentication**, members of that organization can't use Google sign-in — Google sign-ins don't pass through DevStride's MFA challenge, so allowing them would bypass the requirement. Those members continue to sign in with their password and MFA as before.
::
