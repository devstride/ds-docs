...
Do this **first**. DevStride asks for credentials that only exist once you've created the app registration on your side.
...
Create a SAML application in your identity provider and point its assertion consumer service (ACS) at:

```
https://login.devstride.com/saml2/idpresponse
```

Then export your IdP's **metadata** — either a metadata URL or the metadata XML document — to bring back to DevStride.

::alert{type="note" title="You may need one more value from us"}
Some identity providers also require a Service Provider **Entity ID** (audience URI) before they'll let you save the application. That value is specific to the DevStride environment you're on and isn't shown in the admin screen — contact DevStride support and we'll give it to you.
::
...
| --- | --- |
| Selected **Sign in with SSO** with no email typed | *Enter your work email address first — single sign-on uses it to find your organization.* |
| Their domain has no verified provider | *Single sign-on is not set up for that email address. Sign in with your password, or check with your administrator.* |
| Something else went wrong | *Single sign-on is unavailable right now. Sign in with your password, or try again.* |
