...
## Moving items while others are editing

Your organization's item-type rules determine which children a parent can contain. When people move items or change their types at the same time, DevStride checks that the resulting parent–child combination is still allowed. If a competing change prevents the save, refresh the affected items and retry. An invalid combination is refused rather than saved.
