...
- You are an organization **Admin** or **Owner** (admins and owners bypass object-level permission checks).
...
DevStride distinguishes between two organization roles for these features: **Admin/Owner** and **Member**.

- **Members can create perpetual boards** in any folder where they have **Editor** or **Owner** access. Board creation is governed by your access to the parent folder, so a Member with edit access to a folder can add boards there.
- **Cycle boards are effectively admin-only.** Creating a cycle relies on cadences, which are configured in **Configure Settings** (Organization Settings), and that area is only available to Admins and Owners. Members can't reach the cadence configuration, so they can't create cycle boards.
- The top-level **+ Folder** button at the top of the Boards page is available only to Admins and Owners.
- The **Measure Performance** module (analytics) is shown in the sidebar only to Admins and Owners; Members don't see it.

::alert{type="info" title="What governs access"}
Access to boards and folders is enforced by per-object permissions (the user, team, and organization grants described on this page) together with your organization role (Admin/Owner vs. Member). Whether the **Measure Performance** module appears in the sidebar is a visibility behavior tied to your role, rather than a separate access control.
::
