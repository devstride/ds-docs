...
```
main (staging data)
├── phil-main          ← Your primary dev branch
├── phil-feature-auth  ← Feature-specific branch
├── sarah-main         ← Another developer's branch
└── ci-pr-142          ← CI/CD ephemeral branch
```

- **`main`** contains the latest staging data. All new branches fork from it.
- **Developer branches** are isolated — your schema changes and test data don't affect anyone else.
...
Neon branches are **automatically provisioned** when you run `ds setup` or deploy via CI. You don't need to create or delete branches manually — the tooling handles this for you.

- **`ds setup`** creates your `dev-{stage}` branch from `main` and writes the connection string to `.env`
- **`ds deploy up`** (CI) creates or reuses the branch for your stage and stores credentials in Secrets Manager
- **`ds deploy down`** tears down the branch along with all other stage resources
