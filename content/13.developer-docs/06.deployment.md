...
::alert{type="info" title="Cloud stages vs. local stage"}
Cloud deploy stages (`{developer}-{branch}`) are separate from your local stage (`{developer}-local`). Each cloud stage gets its own AWS resources, Neon database branch, and Pulumi stack — fully isolated from your local development environment and from other cloud stages.
::
...
4. **Neon database branch** is kept by default — use `--delete-neon` to also remove it
...
## Pausing and Resuming a Stage

To stop a stage from processing requests without destroying its infrastructure:

```bash
# Pause — set Lambda concurrency to 0 (API stops responding)
ds deploy pause

# Resume — restore Lambda concurrency to normal
ds deploy resume
```

A paused stage costs almost nothing to run. Use this to save costs on stages you're not actively using but want to keep around. The stage can be fully resumed in seconds.

## Reconciling Pulumi State (Drift Fix)
...
::alert{type="info" title="Automatic drift recovery"}
`ds deploy up` already includes automatic drift recovery — it will run `pulumi refresh` and retry automatically if the initial deploy fails due to drift. `ds deploy refresh` is for manual use when you want to inspect or fix state explicitly.
::
