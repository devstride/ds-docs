...
ds setup                        → One-time environment setup
    ↓
ds local run backend/frontend   → Daily local development
    ↓
...
The backend starts on port 3001 and the frontend on port 8080, proxying API requests to the backend.
...
### Check Your Neon Branch

```bash
ds db list-projects
```

Lists all Neon projects and branches accessible with your API key. Use `ds db set target-db` to switch branches if needed.
...
| Deploy frontend only (~60s) | `ds deploy frontend [stage]` |
| Pause stage (save costs) | `ds deploy pause` |
| Resume paused stage | `ds deploy resume` |
| Fix Pulumi drift | `ds deploy refresh` |
...
| Full database reset | `ds db reset` |
| Switch database branch | `ds db set target-db` |
