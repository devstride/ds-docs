...
- **Personal stages:** Named `{developer}-{branch}` (e.g., `phil-main`, `phil-feature-auth`). Created automatically based on your git branch.
...
### Branch-Aware Stage Resolution

The CLI automatically resolves your stage from your git branch:

1. Reads `DEVSTRIDE_DEVELOPER` (your name, set during setup).
2. Reads the current git branch name.
3. Computes stage: `{developer}-{branch}` (lowercase, sanitized).
4. Sets `DEVSTRIDE_STAGE` so all commands target the correct environment.

This means switching git branches automatically targets a different stage — no manual configuration needed.
...
|---|---|
| **Tool Presence** | Node.js, pnpm, AWS CLI, Pulumi, git, Docker, gh |
| **Service Access** | Pulumi Cloud auth, org membership, AWS SSO validity |
