...
You can create multiple cloud instances for different purposes. The CLI automatically detects existing instances and suggests the next available name.

### Auto-Increment (Default)

Simply run `create` again and the CLI will detect existing instances:

```bash
./ds cloud-dev create
# If dev-phil exists, prompts: "Found existing environments: dev-phil. Create new environment 'dev-phil-2'?"

./ds cloud-dev create
# If dev-phil and dev-phil-2 exist, prompts: "Create new environment 'dev-phil-3'?"
```

### Explicit Instance Names

You can also specify an instance identifier explicitly:

```bash
./ds cloud-dev create -i 2          # Create dev-phil-2
./ds cloud-dev create -i feature    # Create dev-phil-feature
```
...
./ds cloud-dev code -i feature

# Stop/start specific instances
./ds cloud-dev stop -i 2
./ds cloud-dev start -i 2
...
- **Parallel work** - Work on multiple tickets simultaneously
