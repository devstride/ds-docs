...
Update the stack to use the new AMI:

```typescript
// In dev-instance.stack.ts
const ami = ec2.MachineImage.genericLinux({
  'us-east-1': 'ami-xxxxxxxxxxxxxxxxx',  // Your AMI ID
});
```
...
The Lambda function in `dev-instance.stack.ts` contains the logic. Key parameters:

```typescript
const cpuThreshold = 5;        // CPU percentage
const idleHours = autoStopHours; // Hours of low CPU before stopping
```
...
Review and restrict as needed in `dev-instance.stack.ts`.
