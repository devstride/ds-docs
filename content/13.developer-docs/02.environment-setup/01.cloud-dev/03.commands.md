...
2. Deploys EC2 instance via SST
3. Waits for instance initialization
4. Configures SSH keys and local config
5. Adds GitHub SSH key automatically (via `gh` CLI)
6. Clones repository and runs `pnpm install`
7. Creates `.env` with shared secrets (uses shared dev database by default)
8. Runs database migrations
9. Optionally starts backend in tmux
