...
## Filters in the "Automations" Section of DevStride
...
1. Navigate to the **Automations** section from the sidebar.
2. Click on the `Filters` button located in the top-right corner of the Automations dashboard.
...
1. Click on the `Filters` button to expand the filters menu.
2. Select the filter you wish to apply, such as Trigger, Action, or Created By.
3. Choose the specific criteria from the dropdown or selection list.
4. The list of automations will update to show only those that match the selected filter(s).
...
2. Click on the `Clear` button inside the filters dropdown to remove all filters at once.
3. Alternatively, click the `X` next to individual filter tags at the top of the automation list to remove them one by one.
